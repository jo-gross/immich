...
import { searchAssetBuilder, withExif, withFaces, withStack } from 'src/utils/database';
...
export class AssetRepository implements IAssetRepository {
  ...
  @ChunkedArray()
  async getByIds(ids: string[], relations?: GetByIdsRelations): Promise<AssetEntity[]> {
    const res = await this.prismaRepository.$kysely
      .selectFrom('assets')
      .selectAll()
      .$if(!!relations?.exifInfo, (qb) => qb.select((eb) => withExif(eb)))
      .$if(!!relations?.faces, (qb) => qb.select((eb) => withFaces(eb)))
      .$if(!!relations?.stack, (qb) => withStack(qb, { assets: !!relations!.stack.assets }))
      .where('id', 'in', ids)
      .execute();
