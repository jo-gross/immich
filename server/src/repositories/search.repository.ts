...
import { asVector, withExif, withFaces, withPeople } from 'src/utils/database';
...
export class SearchRepository implements ISearchRepository {
  ...
  async searchSmart(pagination: SearchPaginationOptions, options: SmartSearchOptions): Paginated<AssetEntity> {
    ...
    await this.prismaRepository.$kysely.transaction().execute(async (tx) => {
      await sql`SET LOCAL vectors.hnsw_ef_search = ${pagination.size + 1}`.execute(tx);
      const builder = this.searchAssetBuilder(options, tx)
  ...
  searchFaces({
    ...
    const vector = asVector(embedding);
    return this.prismaRepository.$kysely.transaction().execute(async (tx) => {
  ...
  async getAssetsByCity(userIds: string[]): Promise<AssetEntity[]> {
    ...
    const { rows } = await sql<AssetEntity>`
    ...
    `.execute(this.prismaRepository.$kysely);
  ...
  setDimensionSize(dimSize: number): Promise<void> {
    ...
    return this.prismaRepository.$kysely.transaction().execute(async (trx) => {
  ...
  async deleteAllSearchEmbeddings(): Promise<void> {
    await sql`TRUNCATE ${sql.table('smart_search')}`.execute(this.prismaRepository.$kysely);
  }
